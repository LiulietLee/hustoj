...
<div class='container'>
    ...
    if (isset($_GET['keyword']) && $_GET['keyword'] != "") {
        ...
        $keyword = "%$keyword%";
        $sql = "SELECT `user_id`,`nick`,`reg_time`,`ip`,`school`,`defunct`, `reading_authority` FROM `users` WHERE (user_id LIKE ?) OR (nick LIKE ?) OR (school LIKE ?) ORDER BY `user_id` DESC";
        $result = pdo_query($sql, $keyword, $keyword, $keyword);
    } else {
        $sql = "SELECT `user_id`,`nick`,`reg_time`,`ip`,`school`,`defunct`, `reading_authority` FROM `users` ORDER BY `reg_time` DESC LIMIT $sid, $idsperpage";
        $result = pdo_query($sql);
    }
    ?>
    ...
    <center>
        <table width=100% border=1 style="text-align:center;">
            ...
            foreach ($result as $row) {
                ...
                echo "<td><form method='get' action='user_ra_change.php'>"
                    . "<input name='reading_authority' style='width: 6.4em' value='". $row['reading_authority']. "'>"
