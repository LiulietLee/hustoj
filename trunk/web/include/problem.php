...
function addproblem($title, $time_limit, $memory_limit, $description, $input, $output, $sample_input, $sample_output, $hint, $source, $spj,$OJ_DATA, $readingAuth) {

//	$spj=($spj);
	
	$sql = "INSERT into `problem` (`title`,`time_limit`,`memory_limit`,
	`description`,`input`,`output`,`sample_input`,`sample_output`,`hint`,`source`,`spj`,`in_date`,`defunct`, `reading_authority`)
	VALUES(?,?,?,?,?,?,?,?,?,?,?,NOW(),'Y', ?)";
	//echo $sql;
	$pid =pdo_query( $sql,$title,$time_limit,$memory_limit,$description,$input,$output,
			$sample_input,$sample_output,$hint,$source,$spj, $readingAuth) ;

//	$sql = "UPDATE problem SET reading_authority=$readingAuth WHERE problem_id=$pid";
//	pdo_query($sql);
